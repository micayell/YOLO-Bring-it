...
export const DrawIt: React.FC<DrawItProps> = ({
  keywords,
  videoRef,
  isGameActive = true,
  onGameComplete,
  ...
  const targetEn = keywords?.en || 'person';
  const [currentPrompt, setCurrentPrompt] = useState("");
  const [showPrompt, setShowPrompt] = useState(false);
  const [promptVisible, setPromptVisible] = useState(false);
  const [gameStarted, setGameStarted] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [gameResult, setGameResult] = useState<'pending' | 'playing' | 'waiting' | 'complete'>('pending');
  ...
  const initializeGame = useCallback(() => {
    ...
    // 랜덤 제시어 선택
        const targetKo = keywords?.ko || '사람';
    setCurrentPrompt(targetKo + '을(를) 그려보세요!');
      ...
      canvas.toBlob(async (b) => {
        ...
        if (response.ok) {
          ...
          if (result.scores && result.best_match && result.best_score_percent) {
            ...
            setTimeout(() => {
              onGameComplete(isCorrect, [], score);
            }, 2000);
          } else {
            setGameResult('waiting');
            setAiResults('그림을 완성했습니다!');
            setTimeout(() => onGameComplete(false, [], 0), 2000);
          }
        } else {
      ...
      <header className="flex flex-col bg-white/50 backdrop-blur-sm border-b border-green-200">
        <div className="bg-white/80 backdrop-blur-sm border-b border-orange-200 p-4">
          <div className="flex justify-between items-center">
            <div>
              <h2 className="text-xl font-bold text-orange-800">DrawIt</h2>
