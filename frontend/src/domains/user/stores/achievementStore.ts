...
import { useItemStore } from "@/domains/user/stores/itemStore"; // ✅ 인벤토리 스토어 불러오기
...
export const useAchievementStore = create<State & Actions>((set, get) => ({
  achievements: [],
  achievementRate: 0,
  loading: false,
  error: "",
  ...
  syncBadBadgesToInventory: () => {
    ...
    const badIds = list
      .filter(
        (a: AchievementCore) => a.hasAchievement && a.item?.itemCategory?.categoryCode === "BAD"
      )
      .map((a: AchievementCore) => String(a.item.itemUid));
