    ...
    >
      <div className="flex items-stretch gap-4 p-4">
        ...
        <div className="w-[200px] flex-shrink-0 flex flex-col justify-between">
          ...
          {completed && onEquip && achievement.item.itemCategory.categoryCode === "BAD" && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                onEquip(achievement);
              }}
              disabled={currentBadge === achievement.item.name}
              className={`mt-2 px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                currentBadge === achievement.item.name
                  ? "bg-muted text-muted-foreground cursor-not-allowed"
                  : "bg-primary text-primary-foreground hover:bg-primary/90"
              }`}
            >
              {currentBadge === achievement.item.name ? "장착됨" : "장착하기"}
            </button>
          )}
        </div>
      </div>
