...
export function ResetPasswordPage() {
  ...
  const email = params.get("email") ?? ""; // /resetpassword?email=...
  ...
  const canSubmit = pw.length >= 8 && pw === pw2 && !!token && !!email && !busy;

  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!token || !email) return setError("유효하지 않은 링크입니다. 메일의 링크로 다시 시도해 주세요.");
    ...
    setBusy(true);
    try {
      await apiClient.post("/users/users/new-password", {
        email,
        token,
        newPassword: pw,
      });
